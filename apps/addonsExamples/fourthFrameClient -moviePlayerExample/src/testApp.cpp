...
void testApp::setup(){
	...
	fingerMovie.loadMovie("YP2011Split4_1.mov");
	//fingerMovie.play();
	connectTime = 0;
	deltaTime = 0;
	firstTime = 0;
...
void testApp::draw(){
	...
	string synFrame = ofToString(fingerMovie.getCurrentFrame());
	ofDrawBitmapString(synFrame,0,10);
	ofDrawBitmapString(synFrame,1240,10);
	*/
}
